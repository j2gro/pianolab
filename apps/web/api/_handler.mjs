...
var apiRoot = join(dirname(fileURLToPath(import.meta.url)), "..");
var repoRoot = resolve(apiRoot, "../..");
...
import { extname, isAbsolute as isAbsolute2, join as join2, relative, resolve as resolve2, sep } from "node:path";
var MIME = {
...
function safeFile(root, pathname) {
  ...
  const abs = resolve2(root, decoded.replace(/^\/+/, ""));
  const rel = relative(root, abs);
  if (!rel || isAbsolute2(rel) || rel.split(sep).includes("..")) {
    return null;
  }
  return abs;
}
...
function tryServeStatic(req, res, pathname, staticRoot) {
...
function signToken(sessionSecret2, userId) {
  const body = Buffer.from(JSON.stringify({ userId, exp: Date.now() + 7 * 24 * 60 * 60 * 1e3 })).toString(
    "base64url"
  );
  const sig = createHmac("sha256", sessionSecret2).update(body).digest("base64url");
  return `${body}.${sig}`;
}
...
function readToken(sessionSecret2, header) {
  ...
  const expected = createHmac("sha256", sessionSecret2).update(body).digest("base64url");
...
function allowOrigin(req, corsOriginEnv) {
...
function send(req, res, corsOriginEnv, status, body) {
...
function parseProgress(body, lessonId) {
  return {
    lessonId,
    hits: Number(body.hits ?? 0),
    wrongs: Number(body.wrongs ?? 0),
    waitsMs: Array.isArray(body.waitsMs) ? body.waitsMs.map(Number) : [],
    completedPhraseIds: Array.isArray(body.completedPhraseIds) ? body.completedPhraseIds.map(String) : [],
...
function createRequestListener(options) {
  const { store, sessionSecret: sessionSecret2, corsOrigin, staticDir } = options;
  const reply = (req, res, status, body) => send(req, res, corsOrigin, status, body);
  return async (req, res) => {
    try {
      ...
      if (req.method === "POST" && pathname === "/auth/register") {
        ...
        reply(req, res, 201, { token: signToken(sessionSecret2, user.id), email: user.email });
        return;
      }
      if (req.method === "POST" && pathname === "/auth/login") {
        ...
        const a2 = Buffer.from(hash);
        const b2 = Buffer.from(user.passwordHash);
        if (a2.length !== b2.length || !timingSafeEqual(a2, b2)) {
          reply(req, res, 401, { error: "invalid_credentials" });
          return;
        }
        reply(req, res, 200, { token: signToken(sessionSecret2, user.id), email: user.email });
        return;
      }
      const isAuthedApi = req.method === "GET" && pathname === "/me" || req.method === "GET" && pathname === "/progress" || req.method === "PUT" && pathname === "/progress";
      if (isAuthedApi) {
        const userId = readToken(sessionSecret2, req.headers.authorization);
        if (!userId) {
        ...
        if (req.method === "GET" && pathname === "/progress") {
          const lessonId2 = url.searchParams.get("lessonId");
          if (!lessonId2) {
            reply(req, res, 400, { error: "lessonId required" });
            return;
          }
          reply(req, res, 200, {
            progress: await store.getProgress(userId, lessonId2)
          });
          return;
        }
        const body = await parseJson(req);
...
// apps/api/src/store-env.ts
import { join as join3 } from "node:path";

// apps/api/src/store-file.ts
...
function createFileStore(storePath) {
  const load = () => {
    ...
    const parsed = JSON.parse(readFileSync3(storePath, "utf8"));
    return {
      users: parsed.users ?? [],
...
// apps/api/src/store-postgres.ts
function withoutChannelBinding(databaseUrl) {
  try {
...
function createPostgresStore(databaseUrl) {
  const sql = cs(withoutChannelBinding(databaseUrl));
  let ready = null;
  const boot = () => {
    ready ??= (async () => {
      await sql`
...
function createStoreFromEnv() {
  ...
  return createFileStore(join3(apiRoot, "data", "store.json"));
}

// apps/api/src/vercel-handler.ts
loadDotEnv();
var sessionSecret = process.env.SESSION_SECRET || "pianolab-poc-dev-secret";
...
var listener = createRequestListener({
  store: createStoreFromEnv(),
  sessionSecret,
  corsOrigin: process.env.CORS_ORIGIN ?? "reflect",
  staticDir: null
});
var vercel_handler_default = listener;
var config = {
  runtime: "nodejs",
  maxDuration: 10
};
